...
int InfiltrationOrgDuengerRegen(schaaf* self)
{
    ...
    // if (pCl->pWeather->fPreciRate > (float)5.0)
    if (pCl->pWeather->fLiquPreciRate > (float)5.0) {
        wirkNied = ((float)1.0 / (float)(sqrt((double)pCl->pWeather->fPreciRate + (double)0.75))) -
                   ((float)0.02 * (pCl->pWeather->fPreciRate - (float)0.25));
        // wirkNied = ((float)1.0/(float)(sqrt((double)pCl->pWeather->fLiquPreciRate + (double)0.75)))
        //					- ((float)0.02 * (pCl->pWeather->fLiquPreciRate - (float)0.25));
        ...
        if (pFE != NULL) {
            if ((!strcmp(pFE->acCode, "RE015\0")) || (!strcmp(pFE->acCode, "RE016\0")) ||
                ...
                if (pCh->pCProfile->fNManureSurf > (float)0.0) {
                    sickorgN = pCh->pCProfile->fNManureSurf * (float)0.02 * ((float)1.0 - wirkNied);
                    pCh->pCProfile->fNManureSurf -= sickorgN;
                    pCh->pCLayer->pNext->fNManure += sickorgN;
                }

                if (pCh->pCProfile->fCManureSurf > (float)0.0) {
                    sickorgC = pCh->pCProfile->fCManureSurf * (float)0.02 * ((float)1.0 - wirkNied);
                    pCh->pCProfile->fCManureSurf -= sickorgC;
                    pCh->pCLayer->pNext->fCManure += sickorgC;
                }
            }
...
int NitrOrgNH4Hoff(schaaf* self)
{
    ...
    for (pCL = pCL->pNext, pSW = pSW->pNext, pSL = pSL->pNext;
        ...
        xpn->pCh->pCLayer->fNH4Manure -= minRate * pTi->pTimeStep->fAct;
        xpn->pCh->pCLayer->fNH4N += minRate * pTi->pTimeStep->fAct;
    }
...
signed short int InfiltrationOrgDuengerBeregnung(EXP_POINTER)
{
    ...
    if ((pCl->pWeather->fLiquPreciRate > (float)5.0) && (pIR != NULL)) {
