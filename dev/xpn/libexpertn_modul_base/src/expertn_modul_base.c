...
// Hong: changed for Scott Demyan et al. 2016.10.06 				
			if (strcmp(pPl->pGenotype->acCropCode,"SB")==0||(strcmp(pPl->pGenotype->acCropCode,"BS")==0))    //Hong: SB/BS=>sugar beet     
                { //Zuckerruebe
                    fCut = (double)0;
                    fHeight = (double)40;
				}

             if ((strcmp(pPl->pGenotype->acCropCode,"RP")==0)||(strcmp(pPl->pGenotype->acCropCode,"RY")==0))
                { //Raps
                   fCut = (double)60;
				  fHeight = (double)160;
                }
