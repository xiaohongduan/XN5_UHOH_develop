...
//MPI Hong
	...
	self->sorted_XPN=(int *)g_malloc0( self->cfg->number_of_xpn * sizeof(int *));
	
	self->XPNforThisRank= (int *)g_malloc0( self->cfg->number_of_xpn * sizeof(int *));
...
// insead of if USE_MP:
//self->multigrid_act=1; // 1= to use, 0 not to use

// 1. arrange the XPN instance according to the cell number and save in "self->XPN_ID_tab"
k = 0;
for (i=0; i<self->cfg->grid_len; i++)
{
for (i2=0; i2<self->cfg->grid[i].instance_len; i2++)
	{
		self->XPN_ID_tab[(self->cfg->grid[i].j*self->matrix_size_y+self->cfg->grid[i].i)+(self->cfg->grid[i].id)*oneGridsize]=k;
		k++;
	}
}
...
// ab hier: only if USE_MPI ########################################################


#ifdef USE_MPI
{
    int groups, XPN_max, XPN;

	groups= self->cfg->number_of_xpn/self->grid_layers;
	XPN_max= (groups%self->numtasks+1)*self->grid_layers;
    self->XPNforThisRank=array_INI(XPN_max+1);
	...
	for (gridId=0; gridId< self->grid_layers;gridId++)
		...
		{
			self->loop_min=(int)((long)(self->rank+0)*(long)(groups)/(long)(self->numtasks))+groups*gridId;
			self->loop_max=(int)((long)(self->rank+1)*(long)(groups)/(long)(self->numtasks))+groups*gridId;
			
			for (g=0;g<self->cfg->number_of_xpn; g++)
				
				{
					if ((g>=self->loop_min)&&(g<self->loop_max))
					{
					XPN=self->sorted_XPN[g];
					self->XPNforThisRank[k]=XPN;
					//memcpy(self->XPNforThisRank[k], self->sorted_XPN[g],sizeof(int)*10);
					k++;
					}					
				}							
			}
...
// replace if ((i3 >= self->loop_min) && (i3 < self->loop_max)): ////////////////////////////
					...
					if (isXPNforThisRank(i3, self->XPNforThisRank, sizeof(self->XPNforThisRank)))
					{ 
#endif						
////////////////////////////////////////////////////////////////////////////////////////////
...
/*							
// Begin of Hong: ausschalten
STRUCT_mpmas_to_xn* See_mpmas_to_xn;
See_mpmas_to_xn = self->grid_mpmas_to_xn;
//End of Hong							
*/							
...
#ifdef USE_MPI
{
	...
	groups= self->cfg->number_of_xpn/self->grid_layers;
	XPN_max= (groups%self->numtasks+1)*self->grid_layers;
    self->XPNforThisRank=array_INI(XPN_max);
...
#ifdef USE_MPI
                           i4=0;
					       if (isXPNforThisRank(i4, self->XPNforThisRank, sizeof(self->XPNforThisRank)))
					        {
...
/* //HONG MPI: ersetzt durch unten ""Rank der anderen"
#ifdef USE_MPI
...
////////////////////////////////////////////////////////////////////////////////MPI Hong begin    
	// Hong: Rank der anderen
    int r,l,n, z, last_n;
	int group,  size;
    int *XPNforRanks, *recordRank, reports[10];
	
	group= self->cfg->number_of_xpn/self->grid_layers;
	
    XPNforRanks=array_INI(self->cfg->number_of_xpn);
	
	recordRank=array_INI(self->cfg->number_of_xpn);
	...
	for (r=0; r<self->numtasks; r++)
		...
		{
			// for r=0
			//1. find the XPNforThisRank
			n=0;
	        for (gridId=0; gridId< self->grid_layers;gridId++)		
		    {
			self->loop_min=(int)((long)(r+0)*(long)(group)/(long)(self->numtasks))+group*gridId;
			self->loop_max=(int)((long)(r+1)*(long)(group)/(long)(self->numtasks))+group*gridId;
			...
			//2. loop XPNforThisRank[i] to record its rank
            for (l=last_n; l<n+last_n; l++)				
			{	
				z= XPNforRanks[l];
				if(z!=(-1))
				{			
				self->xpn_class[z]->pXSys->MPI_rank=r;
				self->xpn_class[z]->pXSys->MPI_size=self->numtasks;
				self->xpn_class[z]->pXSys->MPI_ranks=(int*)g_malloc0(sizeof(int)*self->cfg->number_of_xpn); 
				// ?
		...
		{
		memcpy(self->xpn_class[i]->pXSys->MPI_ranks, recordRank, self->cfg->number_of_xpn);
...
int isXPNforThisRank(int val, int *arr, int size){
    int i, number;
    for (i=0; i < size; i++) {
		number = arr[i];
        if (number == val)
